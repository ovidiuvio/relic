...
@pytest.fixture
def bookmarkable_relic(http, registered_user):
...
def test_add_bookmark(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    relic_id = bookmarkable_relic

    resp = http.post(f"/api/v1/bookmarks?relic_id={relic_id}", headers={"X-User-Key": key})
    assert resp.status_code == 200
    data = resp.json()
    ...
    # Cleanup
    http.delete(f"/api/v1/bookmarks/{relic_id}", headers={"X-User-Key": key})
...
@pytest.mark.integration
def test_add_bookmark_not_found(http, registered_user):
    key, _ = registered_user
    resp = http.post("/api/v1/bookmarks?relic_id=nonexistent_relic_bm", headers={"X-User-Key": key})
    assert resp.status_code == 404
    assert resp.json()["detail"] == "Relic not found"


@pytest.mark.integration
def test_add_bookmark_already_exists(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    relic_id = bookmarkable_relic

    http.post(f"/api/v1/bookmarks?relic_id={relic_id}", headers={"X-User-Key": key})
    resp = http.post(f"/api/v1/bookmarks?relic_id={relic_id}", headers={"X-User-Key": key})
...
@pytest.mark.integration
def test_remove_bookmark(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    relic_id = bookmarkable_relic

    http.post(f"/api/v1/bookmarks?relic_id={relic_id}", headers={"X-User-Key": key})
    resp = http.delete(f"/api/v1/bookmarks/{relic_id}", headers={"X-User-Key": key})
    assert resp.status_code == 200
...
@pytest.mark.integration
def test_remove_bookmark_not_found(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    resp = http.delete(f"/api/v1/bookmarks/{bookmarkable_relic}", headers={"X-User-Key": key})
    assert resp.status_code == 404
...
@pytest.mark.integration
def test_check_bookmark_true(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    relic_id = bookmarkable_relic

    bm = http.post(f"/api/v1/bookmarks?relic_id={relic_id}", headers={"X-User-Key": key}).json()
    resp = http.get(f"/api/v1/bookmarks/check/{relic_id}", headers={"X-User-Key": key})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.integration
def test_check_bookmark_false(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    resp = http.get(f"/api/v1/bookmarks/check/{bookmarkable_relic}", headers={"X-User-Key": key})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.integration
def test_get_user_bookmarks(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    relic_id = bookmarkable_relic

    bm = http.post(f"/api/v1/bookmarks?relic_id={relic_id}", headers={"X-User-Key": key}).json()
    resp = http.get("/api/v1/bookmarks", headers={"X-User-Key": key})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.integration
def test_get_user_bookmarks_unauthorized(http):
...
@pytest.mark.integration
def test_get_relic_bookmarkers(http, registered_user, bookmarkable_relic):
    key, _ = registered_user
    relic_id = bookmarkable_relic
