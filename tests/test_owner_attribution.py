...
owner's client_key row via joinedload(Relic.owner_client). They are surfaced
...
def named_owner(http):
    """A registered client with a display name set. Returns (key, public_id, name)."""
    key = uuid.uuid4().hex
    reg = http.post("/api/v1/client/register", headers={"X-Client-Key": key})
...
def owned_relic(http, named_owner):
    """A public relic owned by the named client. Cleans up after."""
...
@pytest.mark.integration
def test_client_relics_includes_owner(http, owned_relic):
    relic_id, key, public_id, name = owned_relic
    data = http.get("/api/v1/client/relics", headers={"X-Client-Key": key}).json()
    relic = next((r for r in data["relics"] if r["id"] == relic_id), None)
...
def test_bookmarks_include_owner(http, owned_relic):
    ...
    http.post("/api/v1/client/register", headers={"X-Client-Key": bookmarker})
    add = http.post(
        f"/api/v1/bookmarks?relic_id={relic_id}",
        headers={"X-Client-Key": bookmarker},
    )
    assert add.status_code == 200

    data = http.get("/api/v1/bookmarks", headers={"X-Client-Key": bookmarker}).json()
...
def test_space_relics_include_owner(http, owned_relic):
    ...
    try:
        ...
        data = http.get(
            f"/api/v1/spaces/{space_id}/relics", headers={"X-Client-Key": key}
        ).json()
        relic = next((r for r in data["relics"] if r["id"] == relic_id), None)
        assert relic is not None
        assert relic["owner_name"] == name
...
def test_anonymous_relic_has_null_owner(http):
    """A relic with no client association reports null owner fields, not an error."""
    resp = http.post(
        "/api/v1/relics",
