...
// tokens (type: tag: by:) that list takes. Pages without a list search Recent.
import { FILTER_KEYS } from "../search/query";

const RELIC_FILTERS = FILTER_KEYS; // type, tag, by
...
export const MINE = { key: "my-relics", label: "My relics", icon: "user", placeholder: "Search your relics", path: "/my-relics", filters: ["type", "tag"] };
  ...
  switch (section) {
    ...
    case "admin":
      return props.tab === "users"
        ...
        : { key: "admin-relics", label: "All relics", icon: "shield", placeholder: "Search every relic", path: "/admin/relics", filters: ["type", "tag"] };
    case "space-view":
