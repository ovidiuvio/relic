<script>
  // The owner, exact-type, date and size filters as page-bar chips, each with × to clear it. They
  // come from the URL (?owner= a public ID, ?type= a content type, ?after= ?before= ?size=), set
  // by clicking an owner's name or a type badge in the list, or by typing them in the search bar.
  // The owner's name comes from the rows they own.
